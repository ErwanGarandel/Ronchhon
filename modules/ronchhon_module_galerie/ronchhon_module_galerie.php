...
public function __construct()
		...
		if(empty($gal_id) || empty($genre_oeuvre_id) || empty($gal_nom)){
			...
			if($operation == 'update'){
				...
				if($max != $gal_id){
					...
					$chg=$this->bdd->prepare('Insert into ron_galerie (gal_id, gal_nom, gal_description, gal_date_r, genre_oeuvre_id) values (:id, :nom, :description, :date_r, :genre)');
					$chg->execute(array(
						':id' => $gal_id,
						':nom' => $gal_nom,
						':description' => $gal_description,
	...
	public function listeOeuvreGalerie(){
		$resRequete = $this->bdd->query('Select gal_id, genre_oeuvre_id, gal_nom, gal_description, gal_date_r, gal_format, gal_chemin from ron_galerie');
	...
	private function getGalerieValues(){
		$resRequete = $this->bdd->query('SELECT gal_id, genre_oeuvre_id, gal_nom, gal_description, gal_date_r, gal_chemin, gal_format FROM ron_galerie where gal_id = '.Tools::getValue('gal_id'));
		$content = $resRequete->fetchAll(PDO::FETCH_ASSOC);
		return $content[0];
	}
